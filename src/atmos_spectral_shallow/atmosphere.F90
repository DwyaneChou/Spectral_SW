...
real    :: total_energy, enstrophy, div_squared, max_Froude
real, dimension(size(Dyn%Grid%u,1), size(Dyn%Grid%u,2)) :: speed,energy

energy = 0.5*(Dyn%Grid%h(:,:,current)*Dyn%Grid%u(:,:,current)**2 + Dyn%Grid%h(:,:,current)**2)
total_energy = &
area_weighted_global_mean(energy)
...
if(root) then
  write(*,1000) day, second, total_energy, enstrophy, div_squared, max_Froude
end if
1000 format(1x, 'day =',i6,2x,'second =', i6, &
    2x,'energy = ',e13.6, 2x,'enstrophy = ',e13.6,3x,'div_squared = ',e13.6, 3x, &
    'max_Froude = ', e10.3)
